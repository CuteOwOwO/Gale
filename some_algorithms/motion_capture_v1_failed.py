...
VIDEO_PATH = "old.mp4"
OUTPUT_DIR = "motion_capture_results"

if not os.path.exists(OUTPUT_DIR):
    os.makedirs(OUTPUT_DIR)
    print(f"[系統] 存檔資料夾已準備: {OUTPUT_DIR}")

def save_detected_frame(frame, prefix):
    if frame is None:
        return
    ...
    def process(self, landmarks, frame):
        ...
        if len(self.right_elbow_angles) == self.window_size:
            ...
            if is_elbow_peak or is_knee_valley:
                return True, self.frame_buffer[mid_idx] # 回傳造成峰值的那一幀
        ...
        while cap.isOpened():
            ...
            if algorithm == "GEOMETRIC":
                ...
                if results.pose_landmarks:
                    ...
                    # 演算法判斷
                    is_peak, peak_frame = geo_detector.process(results.pose_landmarks.landmark, frame)
                    
                    if is_peak:
                        detected = True
                        info_text = "Pose Peak Detected!"
                        save_detected_frame(peak_frame, "geometric")
                ...
                if is_static:
                    ...
                    save_detected_frame(raw_frame, "optical")
